<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="限制人数" align="center" width="100">
        <template slot-scope="{ row }">
          <span>{{ row.s_num }}</span>
        </template>
      </el-table-column>
      <el-table-column label="已使用人数" align="center" width="100">
        <template slot-scope="{ row }">
          <span>{{ row.used_num }}</span>
        </template>
      </el-table-column>
      <el-table-column label="秒杀状态" align="center" width="100">
        <template slot-scope="{ row }">
          <span>{{ row.time_status }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchFlashsale(this.listQuery).then((res) => {
        console.log(res);
        if (res.code === 20000) {
