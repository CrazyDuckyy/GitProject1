<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="优惠券ID" align="center" width="100">
        <template slot-scope="{ row }">
          <span>{{ row.id }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="适用课程" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.value.title }}</span>
        </template>
      </el-table-column>
      <el-table-column label="使用期限" align="center" width="150">
        <template slot-scope="{ row }">
          <span>{{ row.start_time }}</span> 至
          <span>{{ row.end_time }}</span>
        </template>
      </el-table-column>
      <el-table-column label="发行量" align="center" width="100">
        <template slot-scope="{ row }">
          <span>{{ row.c_num }}</span>
        </template>
      </el-table-column>
      <el-table-column label="已领取" align="center" width="100">
        <template slot-scope="{ row }">
          <span>{{ row.received_num }}</span>
        </template>
      </el-table-column>
      <el-table-column label="已使用" align="center" width="100">
        <template slot-scope="{ row }">
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchCoupon(this.listQuery).then((res) => {
        console.log(res);
        if (res.code === 20000) {
